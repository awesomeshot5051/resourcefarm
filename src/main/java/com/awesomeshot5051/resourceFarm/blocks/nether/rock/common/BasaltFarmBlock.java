...
public class BasaltFarmBlock extends BlockBase implements EntityBlock, IItemBlock {
    ...
    public Item toItem() {
        return new CustomRendererBlockItem(this, new Item.Properties()) {
            ...
            public ItemRenderer createItemRenderer() {
                return new BasaltFarmItemRenderer();
            }
        };
    }

    private void playSound(@NotNull Level level, BlockState state, SoundEvent sound, BasaltFarmTileentity farm) {
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        BasaltFarmTileentity trader = VillagerBlockEntityData.getAndStoreBlockEntity(stack, context.registries(), context.level(), () -> new BasaltFarmTileentity(BlockPos.ZERO, ModBlocks.BASALT_FARM.get().defaultBlockState()));
        BasaltFarmTileentity trader2 = FarmBlockEntityData.getAndStoreBlockEntity(stack, context.registries(), context.level(), () -> new BasaltFarmTileentity(BlockPos.ZERO, ModBlocks.BASALT_FARM.get().defaultBlockState()));
    }
    ...
    public void setPlacedBy(Level level, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        ...
        if (blockEntity instanceof BasaltFarmTileentity farmTileEntity) {
    ...
    protected ItemInteractionResult useItemOn(ItemStack heldItem, BlockState state, Level worldIn, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult hit) {
        ...
        if (!(tileEntity instanceof BasaltFarmTileentity farm)) {
        ...
        player.openMenu(new MenuProvider() {
            ...
            public AbstractContainerMenu createMenu(int id, Inventory playerInventory, Player player) {
                return new OutputContainer(id, playerInventory, farm.getOutputInventory(), ContainerLevelAccess.create(worldIn, pos), ModBlocks.BASALT_FARM::get);
            }
        });
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        return new BasaltFarmTileentity(blockPos, blockState); // Spawn EndermanFarmTileentity
    }
