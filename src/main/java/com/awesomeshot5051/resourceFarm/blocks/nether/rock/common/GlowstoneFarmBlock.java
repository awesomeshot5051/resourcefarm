...
public class GlowstoneFarmBlock extends BlockBase implements EntityBlock, IItemBlock {

    public static final EnumProperty<PickaxeType> PICKAXE_TYPE = EnumProperty.create("pickaxe_type", PickaxeType.class);

    public GlowstoneFarmBlock() {
        super(Properties.of().mapColor(MapColor.STONE).strength(2.5F).sound(SoundType.STONE).noOcclusion());
    }

    private void playSound(@NotNull Level level, BlockState state, SoundEvent sound, GlowstoneFarmTileentity farm) {
    ...
    public Item toItem() {
        return new CustomRendererBlockItem(this, new Item.Properties()) {
            ...
            public ItemRenderer createItemRenderer() {
                return new GlowstoneFarmItemRenderer();
            }
        };
    }
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        GlowstoneFarmTileentity trader = VillagerBlockEntityData.getAndStoreBlockEntity(stack, context.registries(), context.level(), () -> new GlowstoneFarmTileentity(BlockPos.ZERO, ModBlocks.GLOWSTONE_FARM.get().defaultBlockState()));
        GlowstoneFarmTileentity trader2 = FarmBlockEntityData.getAndStoreBlockEntity(stack, context.registries(), context.level(), () -> new GlowstoneFarmTileentity(BlockPos.ZERO, ModBlocks.GLOWSTONE_FARM.get().defaultBlockState()));
    }
    ...
    public void setPlacedBy(Level level, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        ...
        if (blockEntity instanceof GlowstoneFarmTileentity farmTileEntity) {
    ...
    protected ItemInteractionResult useItemOn(ItemStack heldItem, BlockState state, Level worldIn, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult hit) {
        ...
        if (!(tileEntity instanceof GlowstoneFarmTileentity farm)) {
        ...
        player.openMenu(new MenuProvider() {
            ...
            public AbstractContainerMenu createMenu(int id, Inventory playerInventory, Player player) {
                return new OutputContainer(id, playerInventory, farm.getOutputInventory(), ContainerLevelAccess.create(worldIn, pos), ModBlocks.GLOWSTONE_FARM::get);
            }
        });
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        return new GlowstoneFarmTileentity(blockPos, blockState); // Spawn EndermanFarmTileentity
    }
