...
public class CobblestoneFarmBlock extends BlockBase implements EntityBlock, IItemBlock {
    ...
    public CobblestoneFarmBlock() {
        super(Properties.of().mapColor(MapColor.STONE).strength(2.5F).sound(SoundType.STONE).noOcclusion());
    }

    private void playSound(@NotNull Level level, BlockState state, SoundEvent sound, CobblestoneFarmTileentity farm) {
    ...
    public Item toItem() {
        return new CustomRendererBlockItem(this, new Item.Properties()) {
            ...
            public ItemRenderer createItemRenderer() {
                return new CobblestoneFarmItemRenderer();
            }
        };
    }
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        CobblestoneFarmTileentity trader = VillagerBlockEntityData.getAndStoreBlockEntity(stack, context.registries(), context.level(), () -> new CobblestoneFarmTileentity(BlockPos.ZERO, ModBlocks.COBBLESTONE_FARM.get().defaultBlockState()));
        CobblestoneFarmTileentity trader2 = FarmBlockEntityData.getAndStoreBlockEntity(stack, context.registries(), context.level(), () -> new CobblestoneFarmTileentity(BlockPos.ZERO, ModBlocks.COBBLESTONE_FARM.get().defaultBlockState()));
    }
    ...
    public void setPlacedBy(Level level, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        ...
        if (blockEntity instanceof CobblestoneFarmTileentity farmTileEntity) {
    ...
    protected ItemInteractionResult useItemOn(ItemStack heldItem, BlockState state, Level worldIn, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult hit) {
        ...
        if (!(tileEntity instanceof CobblestoneFarmTileentity farm)) {
        ...
        player.openMenu(new MenuProvider() {
            ...
            public AbstractContainerMenu createMenu(int id, Inventory playerInventory, Player player) {
                return new OutputContainer(id, playerInventory, farm.getOutputInventory(), ContainerLevelAccess.create(worldIn, pos), ModBlocks.COBBLESTONE_FARM::get);
            }
        });
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        return new CobblestoneFarmTileentity(blockPos, blockState); // Spawn EndermanFarmTileentity
    }
