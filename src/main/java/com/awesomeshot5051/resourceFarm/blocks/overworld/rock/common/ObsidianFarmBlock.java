...
public class ObsidianFarmBlock extends BlockBase implements EntityBlock, IItemBlock {

    public static final EnumProperty<PickaxeType> PICKAXE_TYPE = EnumProperty.create("pickaxe_type", PickaxeType.class);

    public ObsidianFarmBlock() {
        super(Properties.of().mapColor(MapColor.STONE).strength(2.5F).sound(SoundType.STONE).noOcclusion());
    }

    private void playSound(@NotNull Level level, BlockState state, SoundEvent sound, ObsidianFarmTileentity farm) {
    ...
    public Item toItem() {
        return new CustomRendererBlockItem(this, new Item.Properties()) {
            ...
            public ItemRenderer createItemRenderer() {
                return new ObsidianFarmItemRenderer();
            }
        };
    }
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        ObsidianFarmTileentity trader = VillagerBlockEntityData.getAndStoreBlockEntity(stack, context.registries(), context.level(), () -> new ObsidianFarmTileentity(BlockPos.ZERO, ModBlocks.OBSIDIAN_FARM.get().defaultBlockState()));
        ObsidianFarmTileentity trader2 = FarmBlockEntityData.getAndStoreBlockEntity(stack, context.registries(), context.level(), () -> new ObsidianFarmTileentity(BlockPos.ZERO, ModBlocks.OBSIDIAN_FARM.get().defaultBlockState()));
    }
    ...
    public void setPlacedBy(Level level, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        ...
        if (blockEntity instanceof ObsidianFarmTileentity farmTileEntity) {
    ...
    protected ItemInteractionResult useItemOn(ItemStack heldItem, BlockState state, Level worldIn, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult hit) {
        ...
        if (!(tileEntity instanceof ObsidianFarmTileentity farm)) {
        ...
        player.openMenu(new MenuProvider() {
            ...
            public AbstractContainerMenu createMenu(int id, Inventory playerInventory, Player player) {
                return new OutputContainer(id, playerInventory, farm.getOutputInventory(), ContainerLevelAccess.create(worldIn, pos), ModBlocks.OBSIDIAN_FARM::get);
            }
        });
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        return new ObsidianFarmTileentity(blockPos, blockState); // Spawn EndermanFarmTileentity
    }
