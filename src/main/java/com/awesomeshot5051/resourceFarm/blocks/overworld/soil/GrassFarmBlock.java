...
public class GrassFarmBlock extends BlockBase implements EntityBlock {

    public static final EnumProperty<PickaxeType> PICKAXE_TYPE = EnumProperty.create("pickaxe_type", PickaxeType.class);

    public GrassFarmBlock(Properties properties) {
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        GrassFarmTileentity trader2 = FarmBlockEntityData.getAndStoreBlockEntity(stack, context.registries(), context.level(), () -> new GrassFarmTileentity(BlockPos.ZERO, ModBlocks.GRASS_FARM.get().defaultBlockState()));
    }
    ...
    public void setPlacedBy(Level level, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        ...
        if (blockEntity instanceof GrassFarmTileentity farmTileEntity) {
    ...
    protected InteractionResult useItemOn(ItemStack heldItem, BlockState state, Level worldIn, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult hit) {
        ...
        if (!(tileEntity instanceof GrassFarmTileentity farm)) {
        ...
        player.openMenu(new MenuProvider() {
            ...
            public AbstractContainerMenu createMenu(int id, Inventory playerInventory, Player player) {
                return new OutputContainer(id, playerInventory, farm.getOutputInventory(), ContainerLevelAccess.create(worldIn, pos), ModBlocks.GRASS_FARM::get);
            }
        });
