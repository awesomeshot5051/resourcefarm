package com.awesomeshot5051.resourceFarm.blocks.tileentity.nether.ores.common.regular;
...
public class NetherQuartzOreFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public NetherQuartzOreFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.NETHER_QUARTZ_FARM.get(), ModBlocks.NETHER_QUARTZ_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getNetherQuartzGenerateTime(NetherQuartzOreFarmTileentity tileEntity) {
    ...
    public static double getNetherQuartzBreakTime(NetherQuartzOreFarmTileentity tileEntity) {
        ...
        return getNetherQuartzGenerateTime(tileEntity) + (pickAxe.equals(PickaxeType.NETHERITE) ? (baseValue * 3.2) :
    ...
    public void tick() {
        timer++;
        if (timer >= getNetherQuartzBreakTime(this)) {
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.QUARTZ, dropCount)); // Change this as needed for custom loot
        if (getPickaxeEnchantmentStatus(pickaxeEnchantments, Enchantments.SILK_TOUCH)) {
            drops.clear();
            drops.add(new ItemStack(Items.NETHER_QUARTZ_ORE, 1));
        }
        return drops;
    }
