...
public class BasaltFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public BasaltFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.BASALT_FARM.get(), ModBlocks.BASALT_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getBasaltBreakTime(BasaltFarmTileentity tileEntity) {

        return getBasaltGenerateTime(tileEntity) + (tileEntity.getPickType().getItem().equals(Items.STONE_PICKAXE) ? (20 * 8) :
    ...
    public void tick() {
        timer++;
        if (timer >= getBasaltBreakTime(this)) {
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.BASALT)); // Change this as needed for custom loot
        return drops;
    }
