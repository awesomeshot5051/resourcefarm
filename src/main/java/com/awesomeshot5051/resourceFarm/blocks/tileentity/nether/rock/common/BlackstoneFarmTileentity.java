...
public class BlackstoneFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public BlackstoneFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.BLACKSTONE_FARM.get(), ModBlocks.BLACKSTONE_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getBlackstoneGenerateTime(BlackstoneFarmTileentity tileEntity) {
    ...
    public static double getBlackstoneBreakTime(BlackstoneFarmTileentity tileEntity) {

        return getBlackstoneGenerateTime(tileEntity) + (tileEntity.getPickType().getItem().equals(Items.STONE_PICKAXE) ? (20 * 8) :
    ...
    public void tick() {
        timer++;
        if (timer >= getBlackstoneBreakTime(this)) {
...
if (getPickaxeEnchantmentStatus(pickaxeEnchantments, Enchantments.FORTUNE)) {
        ...
        drops.add(dropCount, new ItemStack(Items.BLACKSTONE)); // Change this as needed for custom loot
        return drops;
    }
