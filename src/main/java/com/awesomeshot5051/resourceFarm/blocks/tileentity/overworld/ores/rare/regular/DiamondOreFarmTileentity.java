package com.awesomeshot5051.resourceFarm.blocks.tileentity.overworld.ores.rare.regular;
...
public class DiamondOreFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public DiamondOreFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.DIAMOND_FARM.get(), ModBlocks.DIAMOND_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getDiamondGenerateTime(DiamondOreFarmTileentity tileEntity) {
    ...
    public static double getDiamondBreakTime(DiamondOreFarmTileentity tileEntity) {
...
//
        return getDiamondGenerateTime(tileEntity) + (pickAxe.equals(PickaxeType.NETHERITE) ? (baseValue * 8) :
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.DIAMOND, dropCount)); // Change this as needed for custom loot
        if (getPickaxeEnchantmentStatus(pickaxeEnchantments, Enchantments.SILK_TOUCH)) {
            drops.clear();
            drops.add(new ItemStack(Items.DIAMOND_ORE));
        }
        return drops;
    }
