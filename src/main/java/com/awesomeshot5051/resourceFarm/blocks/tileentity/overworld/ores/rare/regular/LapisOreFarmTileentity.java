...
public class LapisOreFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public LapisOreFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.LAPIS_FARM.get(), ModBlocks.LAPIS_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getLapisGenerateTime(LapisOreFarmTileentity tileEntity) {
    ...
    public static double getLapisBreakTime(LapisOreFarmTileentity tileEntity) {

        return getLapisGenerateTime(tileEntity) + (tileEntity.getPickType().getItem().equals(Items.IRON_PICKAXE) ? (20 * 4) :
    ...
    public void tick() {
        ...
        if (timer >= getLapisBreakTime(this)) {
