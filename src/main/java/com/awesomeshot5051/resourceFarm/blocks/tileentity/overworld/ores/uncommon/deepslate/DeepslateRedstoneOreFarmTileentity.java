package com.awesomeshot5051.resourceFarm.blocks.tileentity.overworld.ores.uncommon.deepslate;
...
public class DeepslateRedstoneOreFarmTileentity extends FarmTileentity implements ITickableBlockEntity {
    ...
    public DeepslateRedstoneOreFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.DREDSTONE_FARM.get(), ModBlocks.DREDSTONE_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getRedstoneBreakTime(DeepslateRedstoneOreFarmTileentity tileEntity) {
        ...
        return getRedstoneGenerateTime(tileEntity) + (pickAxe.equals(PickaxeType.NETHERITE) ? (baseValue * 8) :
    ...
    public void tick() {
        ...
        if (timer >= getRedstoneBreakTime(this)) {
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.REDSTONE, dropCount));
        if (getPickaxeEnchantmentStatus(pickaxeEnchantments, Enchantments.SILK_TOUCH)) {
            drops.clear();
            if (smelterUpgradeEnabled) drops.add(new ItemStack(Items.REDSTONE));
            else drops.add(new ItemStack(Items.DEEPSLATE_REDSTONE_ORE, 1));
        }
        return drops;
    }
