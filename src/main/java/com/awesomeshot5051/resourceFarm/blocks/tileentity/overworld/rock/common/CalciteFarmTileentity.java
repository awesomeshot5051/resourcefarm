...
public class CalciteFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public CalciteFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.CALCITE_FARM.get(), ModBlocks.CALCITE_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getCalciteGenerateTime(CalciteFarmTileentity tileEntity) {
    ...
    public static double getCalciteBreakTime(CalciteFarmTileentity tileEntity) {

        return getCalciteGenerateTime(tileEntity) + (tileEntity.getPickType().getItem().equals(Items.STONE_PICKAXE) ? (20 * 8) :
    ...
    public void tick() {
        timer++;
        if (timer >= getCalciteBreakTime(this)) {
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.CALCITE)); // Change this as needed for custom loot
        return drops;
    }
