...
public class GraniteFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {
    ...
    public GraniteFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.GRANITE_FARM.get(), ModBlocks.GRANITE_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getGraniteGenerateTime(GraniteFarmTileentity tileEntity) {
    ...
    public static double getGraniteBreakTime(GraniteFarmTileentity tileEntity) {

        return getGraniteGenerateTime(tileEntity) + (tileEntity.getPickType().getItem().equals(Items.STONE_PICKAXE) ? (20 * 8) :
    ...
    public void tick() {
        timer++;
        if (timer >= getGraniteBreakTime(this)) {
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(Items.GRANITE)); // Change this as needed for custom loot
        return drops;
    }
