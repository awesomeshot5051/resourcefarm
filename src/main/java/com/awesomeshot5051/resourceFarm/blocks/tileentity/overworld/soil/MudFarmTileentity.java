...
public class MudFarmTileentity extends FarmTileentity implements ITickableBlockEntity {
    ...
    public MudFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.MUD_FARM.get(), ModBlocks.MUD_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getMudGenerateTime(MudFarmTileentity tileEntity) {
    ...
    public static double getMudBreakTime(MudFarmTileentity tileEntity) {
        ...
        return getMudGenerateTime(tileEntity) + (shovel.equals(ShovelType.NETHERITE) ? (baseValue * 8) :
    ...
    private List<ItemStack> getDrops() {
        ...
        if (level.getBlockState(getBlockPos().below(2)).is(Blocks.POINTED_DRIPSTONE)) {
            drops.add(new ItemStack(Items.CLAY, dropCount));
        } else drops.add(new ItemStack(Items.MUD, dropCount));
        return drops;
    }
