package com.awesomeshot5051.resourceFarm.blocks.tileentity.render.end.rock.common;
...
public class EndStoneFarmRenderer extends RendererBase<EndStoneFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public EndStoneFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(EndStoneFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        if (farm.getTimer() >= EndStoneFarmTileentity.getEndStoneGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= EndStoneFarmTileentity.getEndStoneBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
