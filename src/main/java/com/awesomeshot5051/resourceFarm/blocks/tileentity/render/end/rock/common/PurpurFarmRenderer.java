...
public class PurpurFarmRenderer extends RendererBase<PurpurFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public PurpurFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(PurpurFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        if (farm.getTimer() >= PurpurFarmTileentity.getPurpurGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= PurpurFarmTileentity.getPurpurBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
