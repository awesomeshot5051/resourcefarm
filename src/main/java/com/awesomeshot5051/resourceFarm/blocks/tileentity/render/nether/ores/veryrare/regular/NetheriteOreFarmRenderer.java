package com.awesomeshot5051.resourceFarm.blocks.tileentity.render.nether.ores.veryrare.regular;
...
public class NetheriteOreFarmRenderer extends RendererBase<NetheriteOreFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public NetheriteOreFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(NetheriteOreFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        if (farm.getTimer() >= NetheriteOreFarmTileentity.getNetheriteGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= NetheriteOreFarmTileentity.getNetheriteBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
