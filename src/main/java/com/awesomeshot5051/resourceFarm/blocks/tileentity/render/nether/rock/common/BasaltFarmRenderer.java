...
public class BasaltFarmRenderer extends RendererBase<BasaltFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public BasaltFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(BasaltFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        if (farm.getTimer() >= BasaltFarmTileentity.getBasaltGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= BasaltFarmTileentity.getBasaltBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
