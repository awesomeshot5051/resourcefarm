...
public class NetherrackFarmRenderer extends RendererBase<NetherrackFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public NetherrackFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(NetherrackFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        if (farm.getTimer() >= NetherrackFarmTileentity.getNetherrackGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= NetherrackFarmTileentity.getNetherrackBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
