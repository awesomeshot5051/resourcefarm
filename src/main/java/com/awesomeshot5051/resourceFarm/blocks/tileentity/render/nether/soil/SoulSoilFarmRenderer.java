...
public class SoulSoilFarmRenderer extends RendererBase<SoulSoilFarmTileentity> {
    ...
    public SoulSoilFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(SoulSoilFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        if (farm.getTimer() >= SoulSoilFarmTileentity.getSoulSoilGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= SoulSoilFarmTileentity.getSoulSoilBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
