package com.awesomeshot5051.resourceFarm.blocks.tileentity.render.overworld.ores.veryrare.deepslate;
...
public class DeepslateEmeraldOreFarmRenderer extends RendererBase<DeepslateEmeraldOreFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public DeepslateEmeraldOreFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(DeepslateEmeraldOreFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        // Render the Emerald Ore Block
        if (farm.getTimer() >= DeepslateEmeraldOreFarmTileentity.getEmeraldGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= DeepslateEmeraldOreFarmTileentity.getEmeraldBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
