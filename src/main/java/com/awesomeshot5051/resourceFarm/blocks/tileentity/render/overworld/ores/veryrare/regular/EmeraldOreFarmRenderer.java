package com.awesomeshot5051.resourceFarm.blocks.tileentity.render.overworld.ores.veryrare.regular;
...
public class EmeraldOreFarmRenderer extends RendererBase<EmeraldOreFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public EmeraldOreFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(EmeraldOreFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        ...
        if (farm.getTimer() >= EmeraldOreFarmTileentity.getEmeraldGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= EmeraldOreFarmTileentity.getEmeraldBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
