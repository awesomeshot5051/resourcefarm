...
public class AndesiteFarmRenderer extends RendererBase<AndesiteFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public AndesiteFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(AndesiteFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        if (farm.getTimer() >= AndesiteFarmTileentity.getAndesiteGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= AndesiteFarmTileentity.getAndesiteBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
