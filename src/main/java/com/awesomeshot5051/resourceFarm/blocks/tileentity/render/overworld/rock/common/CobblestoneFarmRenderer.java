...
public class CobblestoneFarmRenderer extends RendererBase<CobblestoneFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public CobblestoneFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(CobblestoneFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        if (farm.getTimer() >= CobblestoneFarmTileentity.getStoneGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= CobblestoneFarmTileentity.getStoneBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
