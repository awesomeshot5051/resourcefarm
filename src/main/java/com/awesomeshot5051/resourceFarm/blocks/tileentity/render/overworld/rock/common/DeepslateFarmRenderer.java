...
public class DeepslateFarmRenderer extends RendererBase<DeepslateFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public DeepslateFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(DeepslateFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        if (farm.getTimer() >= DeepslateFarmTileentity.getDeepslateGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= DeepslateFarmTileentity.getDeepslateBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
