...
public class TuffFarmRenderer extends RendererBase<TuffFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public TuffFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(TuffFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        super.render(farm, partialTicks, matrixStack, buffer, combinedLight, combinedOverlay);
        matrixStack.pushPose();
        matrixStack.scale(.5f, .5f, .5f);
        matrixStack.translate(.5, 0, 0.5);
        if (farm.getTimer() >= TuffFarmTileentity.getTuffGenerateTime(farm)) {
        ...
        } else if (farm.getTimer() >= TuffFarmTileentity.getTuffBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
