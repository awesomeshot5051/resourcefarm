...
//    }
    ...
    public void render(FluixCrystalFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        ...
        matrixStack.translate(0, 0, 0);
        renderFluid(farm, new FluidStack(Fluids.WATER, 1000), matrixStack, buffer, getLightLevel(farm.getLevel(), farm.getBlockPos()), combinedOverlay);
        if (farm.getTimer() >= FluixCrystalFarmTileentity.getFluixCrystalGenerateTime(farm) && farm.getTimer() < FluixCrystalFarmTileentity.getFluixCrystalBreakTime(farm)) {
...
//            matrixStack.popPose();
    ...
    private void renderFluid(FluixCrystalFarmTileentity blockEntity, FluidStack fluidStack, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        if (fluidStack.isEmpty()) {
            return;
        }
