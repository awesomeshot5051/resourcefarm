...
public class SkyStoneFarmRenderer extends RendererBase<SkyStoneFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public SkyStoneFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(SkyStoneFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        ...
        if (farm.getTimer() >= SkyStoneFarmTileentity.getCGlassGenerateTime(farm)) {
            if (farm.redstoneUpgradeEnabled && !(level.hasNeighborSignal(farm.getBlockPos()))) {
                ...
                blockRenderDispatcher.renderSingleBlock(
                        AE2Blocks.SKY_STONE_BLOCK.get().defaultBlockState(),
                        matrixStack,
                        buffer,
                        combinedLight,
                        combinedOverlay,
        ...
        } else if (farm.getTimer() >= SkyStoneFarmTileentity.getCGlassBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
                    Blocks.AIR.defaultBlockState(),
                    matrixStack,
                    buffer,
