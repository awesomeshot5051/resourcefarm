...
public class FlawedBuddingQuartzFarmRenderer extends RendererBase<FlawedBuddingQuartzFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public FlawedBuddingQuartzFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(FlawedBuddingQuartzFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        ...
        if (farm.getTimer() >= FlawedBuddingQuartzFarmTileentity.getFBQGenerateTime(farm)) {
            if (farm.redstoneUpgradeEnabled && !(level.hasNeighborSignal(farm.getBlockPos()))) {
                ...
                blockRenderDispatcher.renderSingleBlock(
                        AE2Blocks.FLAWED_BUDDING_QUARTZ.get().defaultBlockState(),
                        matrixStack,
                        buffer,
                        combinedLight,
                        combinedOverlay,
