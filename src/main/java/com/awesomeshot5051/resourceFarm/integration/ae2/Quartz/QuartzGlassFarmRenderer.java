...
public class QuartzGlassFarmRenderer extends RendererBase<QuartzGlassFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public QuartzGlassFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(QuartzGlassFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        ...
        if (farm.getTimer() >= QuartzGlassFarmTileentity.getCGlassGenerateTime(farm)) {
            if (farm.redstoneUpgradeEnabled && !(level.hasNeighborSignal(farm.getBlockPos()))) {
                ...
                blockRenderDispatcher.renderSingleBlock(
                        AE2Blocks.QUARTZ_GLASS.get().defaultBlockState(),
                        matrixStack,
                        buffer,
                        combinedLight,
                        combinedOverlay,
        ...
        } else if (farm.getTimer() >= QuartzGlassFarmTileentity.getCGlassBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
                    Blocks.AIR.defaultBlockState(),
                    matrixStack,
                    buffer,
