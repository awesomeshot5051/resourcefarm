...
public class CrystalChorusBrickFarmRenderer extends RendererBase<CrystalChorusBrickFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public CrystalChorusBrickFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(CrystalChorusBrickFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        ...
        if (farm.getTimer() >= CrystalChorusBrickFarmTileentity.getCCbrickGenerateTime(farm)) {
            if (farm.redstoneUpgradeEnabled && !(level.hasNeighborSignal(farm.getBlockPos()))) {
                ...
                blockRenderDispatcher.renderSingleBlock(
                        BuiltInRegistries.BLOCK.get(ResourceLocation.fromNamespaceAndPath("integrateddynamics", "crystalized_chorus_brick")).defaultBlockState(),
                        matrixStack,
                        buffer,
                        combinedLight,
                        combinedOverlay,
        ...
        } else if (farm.getTimer() >= CrystalChorusBrickFarmTileentity.getCCbrickBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
                    Blocks.AIR.defaultBlockState(),
                    matrixStack,
                    buffer,
