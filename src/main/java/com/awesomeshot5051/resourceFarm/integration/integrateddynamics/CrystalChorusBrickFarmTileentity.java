...
public class CrystalChorusBrickFarmTileentity extends FarmTileentity implements ITickableBlockEntity {
    ...
    public CrystalChorusBrickFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.CCBRICK_FARM.get(), ModBlocks.CCBRICK_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getCCbrickGenerateTime(CrystalChorusBrickFarmTileentity tileEntity) {
    ...
    public static double getCCbrickBreakTime(CrystalChorusBrickFarmTileentity tileEntity) {
        ...
        return getCCbrickGenerateTime(tileEntity) + (pickAxe.equals(PickaxeType.NETHERITE) ? (baseValue * 8) :
    ...
    public void tick() {
        ...
        if (Upgrades.getUpgradeStatus(upgrades, ModItems.REDSTONE_UPGRADE.toStack())) {
            if (!level.hasNeighborSignal(getBlockPos())) {
                return;
            } else if (timer >= getCCbrickBreakTime(this)) {
    ...
    private List<ItemStack> getDrops() {
        ...
        drops.add(new ItemStack(BuiltInRegistries.ITEM.get(ResourceLocation.fromNamespaceAndPath("integrateddynamics", "crystalized_chorus_brick")), dropCount));
        return drops;
    }
