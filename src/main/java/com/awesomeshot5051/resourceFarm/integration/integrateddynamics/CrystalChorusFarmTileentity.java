...
public class CrystalChorusFarmTileentity extends FarmTileentity implements ITickableBlockEntity {
    ...
    public CrystalChorusFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.CCHORUS_FARM.get(), ModBlocks.CCHORUS_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getCChorusGenerateTime(CrystalChorusFarmTileentity tileEntity) {
    ...
    public static double getCChorusBreakTime(CrystalChorusFarmTileentity tileEntity) {
        ...
        return getCChorusGenerateTime(tileEntity) + (pickAxe.equals(PickaxeType.NETHERITE) ? (baseValue * 8) :
    ...
    public void tick() {
        ...
        if (Upgrades.getUpgradeStatus(upgrades, ModItems.REDSTONE_UPGRADE.toStack())) {
            if (!level.hasNeighborSignal(getBlockPos())) {
                return;
            } else if (timer >= getCChorusBreakTime(this)) {
