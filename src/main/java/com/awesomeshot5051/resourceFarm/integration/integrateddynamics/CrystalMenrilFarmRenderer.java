...
public class CrystalMenrilFarmRenderer extends RendererBase<CrystalMenrilFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public CrystalMenrilFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(CrystalMenrilFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        ...
        if (farm.getTimer() >= CrystalMenrilFarmTileentity.getCMenrilGenerateTime(farm)) {
            if (farm.redstoneUpgradeEnabled && !(level.hasNeighborSignal(farm.getBlockPos()))) {
                ...
                blockRenderDispatcher.renderSingleBlock(
                        BuiltInRegistries.BLOCK.get(ResourceLocation.fromNamespaceAndPath("integrateddynamics", "crystalized_menril_block")).defaultBlockState(),
                        matrixStack,
                        buffer,
                        combinedLight,
                        combinedOverlay,
        ...
        } else if (farm.getTimer() >= CrystalMenrilFarmTileentity.getCMenrilBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
                    Blocks.AIR.defaultBlockState(),
                    matrixStack,
                    buffer,
