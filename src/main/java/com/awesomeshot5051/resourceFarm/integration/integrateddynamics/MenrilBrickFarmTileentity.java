...
public class MenrilBrickFarmTileentity extends FarmTileentity implements ITickableBlockEntity {
    ...
    public MenrilBrickFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.MBRICK_FARM.get(), ModBlocks.MBRICK_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getMBrickGenerateTime(MenrilBrickFarmTileentity tileEntity) {
    ...
    public static double getMBrickBreakTime(MenrilBrickFarmTileentity tileEntity) {
        ...
        return getMBrickGenerateTime(tileEntity) + (pickAxe.equals(PickaxeType.NETHERITE) ? (baseValue * 8) :
    ...
    public void tick() {
        ...
        if (Upgrades.getUpgradeStatus(upgrades, ModItems.REDSTONE_UPGRADE.toStack())) {
            if (!level.hasNeighborSignal(getBlockPos())) {
                return;
            } else if (timer >= getMBrickBreakTime(this)) {
