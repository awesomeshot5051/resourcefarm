...
public class MenrilGlassFarmRenderer extends RendererBase<MenrilGlassFarmTileentity> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public MenrilGlassFarmRenderer(BlockEntityRendererProvider.Context renderer) {
        super(renderer);
        this.blockRenderDispatcher = renderer.getBlockRenderDispatcher();
    }

    @Override
    public void render(MenrilGlassFarmTileentity farm, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        ...
        if (farm.getTimer() >= MenrilGlassFarmTileentity.getMGlassGenerateTime(farm)) {
            if (farm.redstoneUpgradeEnabled && !(level.hasNeighborSignal(farm.getBlockPos()))) {
                ...
                blockRenderDispatcher.renderSingleBlock(
                        IDBlocks.MENRIL_GLASS.get().defaultBlockState(),
                        matrixStack,
                        buffer,
                        combinedLight,
                        combinedOverlay,
        ...
        } else if (farm.getTimer() >= MenrilGlassFarmTileentity.getMGlassBreakTime(farm)) {
            blockRenderDispatcher.renderSingleBlock(
                    Blocks.AIR.defaultBlockState(),
                    matrixStack,
                    buffer,
