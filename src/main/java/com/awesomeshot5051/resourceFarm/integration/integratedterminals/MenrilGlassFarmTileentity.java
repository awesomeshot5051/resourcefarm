...
public class MenrilGlassFarmTileentity extends FarmTileentity implements ITickableBlockEntity {
    ...
    public MenrilGlassFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.MGLASS_FARM.get(), ModBlocks.MGLASS_FARM.get().defaultBlockState(), pos, state);
    ...
    public static double getMGlassGenerateTime(MenrilGlassFarmTileentity tileEntity) {
    ...
    public static double getMGlassBreakTime(MenrilGlassFarmTileentity tileEntity) {
        ...
        return getMGlassGenerateTime(tileEntity) + (pickAxe.equals(PickaxeType.NETHERITE) ? (baseValue * 8) :
    ...
    public void tick() {
        ...
        if (Upgrades.getUpgradeStatus(upgrades, ModItems.REDSTONE_UPGRADE.toStack())) {
            if (!level.hasNeighborSignal(getBlockPos())) {
                return;
            } else if (timer >= getMGlassBreakTime(this)) {
